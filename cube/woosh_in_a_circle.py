...
# Initialize the single color generator
...
def draw_slice(slice_index):
    """Draw a single slice with a new color."""
    start_angle = (360 / num_slices) * slice_index
    ...
    pygame.draw.polygon(screen, slice_colors[slice_index], points)
...
while running:
    ...
    # Blit the previous frame
    if previous_surface:
        screen.blit(previous_surface, (0, 0))

    # Update only the current slice color
    current_slice_index = frame % num_slices
    set_slice_color(current_slice_index, next(color_generator))

    # Draw the updated slice
